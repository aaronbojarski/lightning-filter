 ...
 */
int
mirror_filter(struct lf_mirror_worker *mirror_ctx, uint16_t port_id,
		...
		struct rte_mbuf *filtered_pkts[LF_MAX_PKT_BURST])
{
	...
	for (i = 0; i < nb_pkts; i++) {
		...
		if (m == NULL) {
			LF_DISTRIBUTOR_LOG_DP(ERR, "Packet is NULL\n");
			continue;
		}
	...
	nb_fwd = lf_mirror_worker_tx(mirror_ctx, port_id, mirrored_pkts,
			nb_mirrored_pkts);
...
lf_distributor_init(struct lf_distributor *distributor,
		struct lf_setup_port_queue_pair port_queues[RTE_MAX_LCORE]
												   [RTE_MAX_ETHPORTS],
		uint16_t nb_port_queues, bool lf_distributor_lcores[RTE_MAX_LCORE],
		bool lf_worker_lcores[RTE_MAX_LCORE], struct lf_mirror *mirror_ctx,
		struct lf_distributor_worker *workers[LF_MAX_WORKER])
{
	int i;
	uint16_t dist_id, worker_id, worker_counter;
	uint16_t lcore_id, port_id;
	uint16_t dist_lcore_id, worker_lcore_id;
	uint16_t socket_id;
	...
	if (LF_DISTRIBUTOR) {
		...
		RTE_LCORE_FOREACH(lcore_id) {
			RTE_ETH_FOREACH_DEV(port_id) {
				// only queues for distributor lcores should be configured in
				// this case. So we do not need to check for distributor lcores
				// here.
				// TODO(aaronbojarski): the current setup does not necessarily
				// work for multiple devices (port_ids).
				if (port_queues[lcore_id][port_id].rx_queue_id ==
						LF_SETUP_INVALID_ID) {
					continue;
				}
				tx_buffer = new_tx_buffer(rte_lcore_to_socket_id(lcore_id));
				if (tx_buffer == NULL) {
					return -1;
				}
				...
				distributor->distributor_contexts[i].mirror_ctx = &mirror_ctx->workers[lcore_id];
				i++;
			}
		}
	} else {
		...
		RTE_LCORE_FOREACH(lcore_id) {
			RTE_ETH_FOREACH_DEV(port_id) {
				...
				workers[i]->rx_tx.port = (struct lf_distributor_rx_tx_port){
					.rx_port_id = port_id,
					.rx_queue_id = port_queues[lcore_id][port_id].rx_queue_id,
					.tx_port_id = port_id,
					.tx_queue_id = port_queues[lcore_id][port_id].tx_queue_id,
					.tx_buffer = tx_buffer,
				};
				i++;
			}
	...
	dist_id = 0;
	worker_lcore_id = 0;
	RTE_LCORE_FOREACH(dist_lcore_id) {
		if (!lf_distributor_lcores[dist_lcore_id]) {
			continue;
		}
		...
		socket_id = rte_lcore_to_socket_id(dist_lcore_id);

		for (worker_counter = 0; worker_counter < nb_workers_per_distributor;
				++worker_counter) {
			// Need to get next worker lcore
			while (worker_lcore_id < RTE_MAX_LCORE) {
				if (lf_worker_lcores[worker_lcore_id]) {
					break;
				}
				worker_lcore_id++;
			}
			if (worker_lcore_id == RTE_MAX_LCORE) {
				LF_DISTRIBUTOR_LOG(ERR,
						"Could not find another worker lcore.\n");
				return -1;
			}

			/* warn if worker is on another lcore than distributer */
			if (socket_id != rte_lcore_to_socket_id(worker_lcore_id)) {
				LF_DISTRIBUTOR_LOG(WARNING,
						...
						"%d (lcore %d)\n",
						worker_id, rte_lcore_to_socket_id(worker_lcore_id),
						worker_lcore_id, dist_id, socket_id, dist_lcore_id);
			}
...
lf_distributor_main_loop(struct lf_distributor_context *distributor_context)
{
	...
	while (likely(!lf_force_quit)) {
		nb_rx = lf_distributor_rx_port(&distributor_context->queue, distributor_context->mirror_ctx, rx_pkts);
