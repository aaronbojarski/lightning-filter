		...
		struct rte_mbuf *pkts[LF_MAX_PKT_BURST])
{
	...
	rx_port_id = queue->rx_port_id;
	tx_queue_id = queue->tx_queue_id;


	/* Forward packets from the mirror to its port. */
	if (lf_mirror_exists(mirror_ctx->ctx, rx_port_id)) {
		nb_rx = lf_mirror_worker_rx(mirror_ctx, rx_port_id, rx_mirror_pkts,
				LF_MAX_PKT_BURST);
		if (nb_rx > 0) {
			LF_DISTRIBUTOR_LOG_DP(DEBUG,
		...
		nb_fwd = rte_eth_tx_burst(rx_port_id, tx_queue_id, rx_mirror_pkts,
				nb_rx);
		if (nb_fwd < nb_rx) {
	...
	if (lf_mirror_exists(mirror_ctx->ctx, rx_port_id)) {
		nb_pkts = mirror_filter(mirror_ctx, rx_port_id, nb_rx, rx_pkts, pkts);
	} else {
		nb_pkts = nb_rx;
		...
		struct rte_mbuf *rx_pkts[LF_MAX_PKT_BURST])
{
	...
	if (LF_DISTRIBUTOR) {
		...
		nb_pkts = lf_distributor_rx_port(&worker_context->rx_tx.port,
				mirror_ctx, rx_pkts);
	}
	return nb_pkts;
}
